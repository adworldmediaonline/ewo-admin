...
import { Search, Delete, Edit } from '@/svg';
import ErrorMsg from '../common/error-msg';
import { useGetAllContactsQuery, useDeleteContactMutation, useUpdateContactStatusMutation, useUpdateContactPriorityMutation } from '@/redux/contact/contactApi';
...
const ContactTable = () => {
  ...
  // API hooks
  const { data: contactsData, error, isLoading, refetch } = useGetAllContactsQuery(queryParams);
  ...
  const handlePriorityChange = async (id: string, priority: IContact['priority']) => {
    try {
              ...
              <tr key={contact._id} className={`hover:bg-gray-50 ${!contact.isRead ? 'bg-blue-50' : ''}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <select
                    ...
                    onChange={(e) => handleStatusChange(contact._id, e.target.value as IContact['status'])}
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <select
                    ...
                    onChange={(e) => handlePriorityChange(contact._id, e.target.value as IContact['priority'])}
                    className="text-sm border-0 bg-transparent focus:ring-0 p-0"
                  >
                    <option value="low">Low</option>
                    <option value="medium">Medium</option>
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="px-6 py-3 border-t border-gray-200 flex items-center justify-between">
          <div className="text-sm text-gray-700">
            Showing {((pagination.currentPage - 1) * (queryParams.limit || 10)) + 1} to{' '}
            {Math.min(pagination.currentPage * (queryParams.limit || 10), pagination.totalResults)} of{' '}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1"><StatusBadge status={selectedContact.status} /></p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Priority</label>
                    <p className="mt-1"><PriorityBadge priority={selectedContact.priority} /></p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Submitted</label>
                    <p className="mt-1 text-sm text-gray-900">
                      {dayjs(selectedContact.createdAt).format('MMMM DD, YYYY [at] h:mm A')}
                    </p>
                  </div>
                </div>
                ...
                {selectedContact.adminNotes && (
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedContact.adminNotes}</p>
                  </div>
                )}
              </div>
